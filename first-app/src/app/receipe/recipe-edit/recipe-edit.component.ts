...
export class RecipeEditComponent implements OnInit {
  ...
  private initForm() {
    ...
    if (this.editMode) {
      const recipe = this.recipeService.getRecipeById(this.name);
  ...
  onSubmit() {
    ...
    if(this.editMode){
      this.recipeService.updateRecipe(newRecipe,this.name);
    }else{
  ...
  getControls(controlsGroup: string) {
    return (<FormArray>this.recipeForm.get(controlsGroup)).controls;
  }
